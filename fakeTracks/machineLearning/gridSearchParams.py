...
def filterSearchParams(filters):

    if len(filters) > 2:
        print("Need to change length of prodcut to match filters")

    filterList = list(it.product(filters[0], filters[1]))

    return filterList


def makeParams(searchParam, allParams):
    
    params = []
    
    if searchParam == '':
        sys.exit("Need to input grid search parameter")

    elif searchParam == 'delete_elements':
        for x in allParams[6]:
            this_params = allParams
            this_params[6] = x
            print(this_params)
            params.append(this_params)

    elif searchParam == 'filters':
        for x in allParams[0]:
            this_params = allParams
            this_params[0] = x
            print(this_params)
            params.append(this_params)
...
if __name__ == '__main__':
    ...
    filters = filterSearchParams(filters)
    #filters = [32, 16]
    batch_norm = True
    undersampling = -1
    epochs = 100

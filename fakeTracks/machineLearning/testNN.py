...
def loadData(dataDir):
    ...
    #if(oversample!=-1):
    #    print("Number of tracks before oversample: " + str(len(trainTracks)))

    #    oversampling = SMOTE(sampling_strategy = oversample)
    #    trainTracks, trainTruth = oversampling.fit_resample(trainTracks, trainTruth)

    #    print("Number of tracks after oversample: " + str(len(trainTracks)))

    #if(undersample!=1):
    #    print("Number of tracks before undersample: " + str(len(trainTracks)))
    #    #sampling_strategy
    #    undersampling = NearMiss(version=3, n_neighbors=3)
    #    trainTracks, trainTruth = undersampling.fit_resample(trainTracks, trainTruth)

    #    print("Number of tracks after undersample: " + str(len(trainTracks)))

    if(oversample!=1 and undersample!=1):
        print("Number of tracks before SMOTEENN: " + str(len(trainTracks)))
        oversampling = SMOTE(sampling_strategy = oversample)
        sme = SMOTEENN(smote=oversampling)
        trainTracks, trainTruth = sme.fit_resample(trainTracks, trainTruth)
        print("Number of tracks after SMOTEENN: " +str(len(trainTracks)))
...
if __name__ == "__main__":
    ...
    workDir = 'outfakesNN_6layer_SMOTEEN_0p3_' + date.today().strftime('%m_%d')

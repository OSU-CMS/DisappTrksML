...
dataDir = "/store/user/mcarrigan/disappearingTracks/converted_singleElectron2017F_tanh_eSel/"
tag = "0p25_"
...
for info, image in zip(s_infos, s_images):
	...
	eleReco=False
	if(math.fabs(info[4]) < 0.15): eleReco = True
	if(requireThreshold): 
		if(thresholdEnergies(image[2:].flatten(), count, energy) == False): continue
...
# save files	
f1 = signal+'Reco_'+tag+str(fileNum)
f2 = 'e_'+tag+str(fileNum)

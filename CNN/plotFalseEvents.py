...
imageDir = "/store/user/mcarrigan/disappearingTracks/electron_selection_threshold_5gt0p5_V2/"
dataDir = "/home/mcarrigan/scratch0/disTracksML/DisappTrksML/CNN/Test/"
nameStartE = "e_0p25_"
nameStartB = "bkg_0p25_"
tag = ""
saveDir = '/home/mcarrigan/scratch0/disTracksML/plots/falseEvents/fullTest_5gt0p5/'
        ...
        if start_file != files[event] or event == len(files)-1:
            ...
            if len(file_events) <= 25 :
                ...
                for j in range(len(file_events)):
                    eta.append(info[int(file_events[j]),7])
                    phi.append(info[int(file_events[j]),8])
                    if j != 0 and j%5 == 0: row += 1
                    col = int(j%5)
                    print("event num ", file_events[j], " index ", j, " data size ", data.shape)
                    e_img[row, col].imshow(np.tanh(data[int(file_events[j]), :, :, 0]))
                    e_img[row, col].set_title("prob: " + str(pred[evt_counter]))
                    e_img[row, col].annotate("eta = "+str(info[int(file_events[j]),7]), xy=(0,0), xytext=(25,3), color='white')
                    e_img[row, col].annotate("phi = "+str(info[int(file_events[j]),8]), xy=(0,0), xytext=(25,5), color='white')

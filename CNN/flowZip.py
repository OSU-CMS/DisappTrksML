  ...
  def __getitem__(self, idx) :
    ...
    for iFile, file in enumerate(filenamesE):
        ...
        if(iFile == 0 and iFile != lastFile):
            #e_images = np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz'))[indexE[0]:]
            e_images = np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz')['images'])
            e_images = e_images[indexE[0]:]
        elif(iFile == lastFile and iFile != 0):
            #e_images = np.concatenate((e_images,np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz'))[:indexE[1]+1]))
            e_temp = np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz')['images'])
            e_images = np.concatenate((e_images, e_temp[:indexE[1]+1]))
        elif(iFile == 0 and iFile == lastFile):
            #e_images = np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz'))[indexE[0]:indexE[1]+1]
            e_images = np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz')['images'])
            e_images = e_images[indexE[0]:indexE[1]+1]
        elif(iFile != 0 and iFile != lastFile):
            #e_images = np.concatenate((e_images,np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz'))))
            e_temp = np.array(np.load(self.dataDir+'e_'+tag+str(file)+endtag+'.npz')['images'])
    ...
    for iFile, file in enumerate(filenamesBkg):
        if(iFile == 0 and iFile != lastFile):
            #bkg_images = np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz'))[indexBkg[0]:,:]
            bkg_images = np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz')['images'])
            bkg_images = bkg_images[indexBkg[0]:,:]
        elif(iFile == lastFile and iFile != 0):
            #bkg_images = np.concatenate((bkg_images,np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz'))[:indexBkg[1]+1]))
            bkg_temp = np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz')['images'])
            bkg_images = np.concatenate((bkg_images, bkg_temp[:indexBkg[1]+1]))
        elif(iFile == 0 and iFile == lastFile):
            #bkg_images = np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz'))[indexBkg[0]:indexBkg[1]+1]
            bkg_images = np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz')['images'])
            bkg_images = bkg_images[indexBkg[0]:indexBkg[1]+1]
        elif(iFile != 0 and iFile != lastFile):
            #bkg_images = np.concatenate((bkg_images,np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz'))))
            bkg_temp = np.array(np.load(self.dataDir+'bkg_'+tag+str(file)+endtag+'.npz')['images'])
    ...
    print("e shape ", e_images.shape)
    print("b shape ", bkg_images.shape)
    ...
    else: batch_x = np.reshape(batch_x, (batch_size, 40, 40, 3))
...
if __name__ == "__main__":
    ...
    dataDir = "/data/disappearingTracks/electron_selection_threshold_5gt0p5/"
    tag = '0p25_tanh_'
    endtag = ''
    inputLayers = 4
    ...
    model.load_weights(weightsDir+weightsFile+'_lastEpoch.h5')
    ...
    if(run_validate):
        validateMod.validate(model, valFile, outputDir, dataDir, tag, endtag, plotDir)

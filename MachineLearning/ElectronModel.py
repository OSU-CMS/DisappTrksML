...
import pickle
import os
from networkController import NetworkBase
...
import numpy as np
import logging
from generator import Generator, BalancedGenerator
...
class ElectronModel(NetworkBase):
        ...
        def train_model(self, data_directory:str, epochs:int = 10, monitor='val_loss',
                ...
                print(f"data_files: {os.listdir(data_directory)}")
                #inputFiles = glob.glob(data_directory + 'images_*.root.npz')
                inputFiles = os.listdir(data_directory)
                ...
                print(f"Input indices: {inputIndices[:int(nFiles*0.7)]}")

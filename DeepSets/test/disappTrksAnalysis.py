...
def eventSelection(event, fiducial_maps, fiducial_map_cut=-1):
    ...
    for track in event.tracks:
        ...
        if (not track.dRMinBadEcalChannel >= 0.05 or
            ...
            not abs(deltaRToClosestElectron) > 0.15 or
            not abs(deltaRToClosestMuon) > 0.15 or
            not abs(deltaRToClosestTauHad) > 0.15 or
...
def main():
    ...
    chain.Add('condor/2017/images_higgsino_300_1000/hist_*.root')

    print 'Added', chain.GetEntries(), 'events'
    ...
    for event in chain:
        ...
        for i, track in enumerate(event.tracks):
            ...
            # converted_arrays = conversion(event, track)
            # discriminant = model.predict(converted_arrays)

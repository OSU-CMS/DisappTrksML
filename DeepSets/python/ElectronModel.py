...
class ElectronModel(DeepSetsArchitecture):
    ...
    def eventSelectionTraining(self, event):
        ...
        for track in event.tracks:
            if (
                abs(track.eta) >= 2.4
                or track.inGap
                or abs(track.dRMinJet) < 0.5
                or abs(track.deltaRToClosestElectron) < 0.15
                or abs(track.deltaRToClosestMuon) < 0.15
                or abs(track.deltaRToClosestTauHad) < 0.15
            ):
    ...
    def convertAMSBFileToNumpy(self, fileName, selection=None):
        ...
        for event in inputTree:
            if selection is None:
                sys.exit("Pick a selection to apply from ['full', 'training']")
            elif selection == "full":
                eventPasses, trackPasses = self.eventSelectionSignal(event)
            elif selection == "training":
                eventPasses, trackPasses = self.eventSelectionSignal(event)
            else:
                sys.exit("Selection not recognized.")
            ...
            for i, track in enumerate(event.tracks):
                ...
                if not (
                    isGenMatched(event, track, 1000022)
                    or isGenMatched(event, track, 1000024)
                ):
                    continue
        ...
        if len(signal) > 0:
            np.savez_compressed(
                outputFileName, signal=signal, signal_infos=signal_infos
            )
            print("Wrote", outputFileName)
        else:
    ...
    def convertTPFileToNumpy(self, fileName):
        ...
        for event in inputTree:
            (
                ...
                trackPasses,
                trackPassesVeto,
            ) = self.eventSelectionLeptonBackground(event, "electrons")
            if not eventPasses:
                continue
        ...
        if len(tracks) != 0:
            ...
            np.savez_compressed(outputFileName, tracks=tracks, infos=infos)
    ...
    def buildModel(self):
        ...
        print(
            "input shape", self.input_shape, "input track shape", self.track_info_shape
        )
        # Encoder Network
        ...
        for layerSize in self.phi_layers[:-1]:
            ...
            phi_network = Activation(tf.keras.layers.LeakyReLU(alpha=0.01))(phi_network)
        ...
        f_network = Activation(tf.keras.layers.LeakyReLU(alpha=0.01))(f_network)
    ...
    def evaluate_model(self, event, track):
        event_converted = self.convertTrackFromTree(
            event, track, 1
        )  # class_label doesn't matter
        prediction = self.model.predict(
            [
                np.reshape(event_converted["sets"], (1, self.max_hits, 4)),
                np.reshape(
                    event_converted["infos"], (1, len(event_converted["infos"]))
                )[:, self.track_info_indices],
            ]
        )
    ...
    def saveGraph(self):
        ...
        cmsml.tensorflow.save_graph(
            "graph.pb.txt", self.model, variables_to_constants=True
        )

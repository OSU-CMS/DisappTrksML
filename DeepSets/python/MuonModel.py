...
class MuonModel(DeepSetsArchitecture):
	...
	def __init__(self, eta_range=0.25, phi_range=0.25,
				 max_hits=100, slimmed_max_hits=20,
		...
		self.slimmed_max_hits = slimmed_max_hits
	...
	def evaluate_model(self, event, track):
		...
		prediction = self.model.predict([np.reshape(event['sets'], (1, self.max_hits, 4))[:, :self.slimmed_max_hits, :], np.reshape(event['infos'], self.track_info_shape)[:,self.track_info_indices]])

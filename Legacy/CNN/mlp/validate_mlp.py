...
def load_data(files, events, label, dataDir):
    ...
    for iFile, file in enumerate(files):
        ...
        if(iFile == 0 and iFile != lastFile):
            ...
            infos = file['infos'][events[0]:, [7,11,12,13,14]]

        elif(iFile == lastFile and iFile != 0):
            images = np.vstack((images,file['images'][:events[1]+1]))
            infos = np.concatenate((infos,file['infos'][:events[1]+1, [7,11,12,13,14]]))

        elif(iFile == 0 and iFile == lastFile):
            images = file['images'][events[0]:events[1]+1]
            infos = file['infos'][events[0]:events[1]+1, [7,11,12,13,14]]

        elif(iFile != 0 and iFile != lastFile):
            images = np.vstack((images,file['images']))
            infos = np.concatenate((infos,file['infos'][:,[7,11,12,13,14]]))
...
class generator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx) :
        ...
        e_images, e_info = load_data(filenamesE, indexE, 'e_0p25_', self.dataDir)
        bkg_images, bkg_info = load_data(filenamesBkg, indexBkg, 'bkg_0p25_', self.dataDir)
        
        numE = e_images.shape[0]
        numBkg = self.batch_size-numE
        bkg_images = bkg_images[:numBkg]
        bkg_info = bkg_info[:numBkg]
